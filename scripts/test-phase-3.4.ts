...
import { createContractSchema, terminateContractSchema } from "../src/features/contracts/schemas/contract.schema";
import { contractChargeConfigSchema } from "../src/features/contracts/schemas/contract-charge-config.schema";
import { validateContractOverlapService } from "../src/features/contracts/services/validate-contract-overlap.service";
...
async function runPhase34Tests() {
  ...
  try {
    const res = terminateContractSchema.safeParse({
  ...
  try {
    const res = contractChargeConfigSchema.safeParse({
      chargeType: "WIFI" as any,
      chargeMethod: "METERED" as any, // Invalid!
      unitPrice: 100000,
  ...
  try {
    const res = contractChargeConfigSchema.safeParse({
      chargeType: "PARKING" as any,
      chargeMethod: "FREE" as any,
  ...
  try {
    ...
    await validateContractOverlapService(mockTx, {
      roomId: "r-101",
  ...
  try {
    const res = contractChargeConfigSchema.safeParse({
      chargeType: "ELECTRICITY" as any,
      chargeMethod: "METERED" as any,
      unitPrice: 4000,
