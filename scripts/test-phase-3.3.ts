...
import { createTenantSchema } from "../src/features/tenants/schemas/tenant.schema";
import { tenantProfileSelfServiceSchema } from "../src/features/tenants/schemas/tenant-profile-self-service.schema";
import { validateImageFileBuffer } from "../src/server/storage/file-validation";
import { generateCCCDSignedUrl, verifyCCCDSignedUrlParams } from "../src/server/storage/signed-url.service";
...
async function runPhase33Tests() {
  ...
  try {
    const res = createTenantSchema.safeParse({ fullName: "   " });
    if (!res.success) {
  ...
  try {
    const res = createTenantSchema.safeParse({ fullName: "Nguyễn Văn A", phone: "123" });
    if (!res.success) {
  ...
  try {
    ...
    const val = validateImageFileBuffer(svgBuffer, "cccd.svg");
  ...
  try {
    ...
    const val = validateImageFileBuffer(largeBuffer, "cccd.jpg");
  ...
  try {
    ...
    const val = validateImageFileBuffer(fakeBuffer, "fake.jpg");
    if (!val.valid) {
  ...
  try {
    ...
    const val = validateImageFileBuffer(validJpgBuffer, "real.jpg");
  ...
  try {
    ...
    const signed = generateCCCDSignedUrl(tenantId, "FRONT", 300);
    const urlObj = new URL(`http://localhost${signed.url}`);
    const side = urlObj.searchParams.get("side") as "FRONT";
  ...
  try {
    ...
    const isValid = verifyCCCDSignedUrlParams(tenantId, "FRONT", String(pastExpires), "fakesig");
    if (!isValid) {
  ...
  try {
    const keys = Object.keys(tenantProfileSelfServiceSchema.shape);
