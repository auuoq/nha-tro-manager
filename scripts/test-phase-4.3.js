...
async function runPhase43Tests() {
    ...
    try {
        const issuable = (status) => status === "DRAFT";
    ...
    try {
        const cancelable = (status) => status === "ISSUED" || status === "OVERDUE";
    ...
    try {
        const validateReason = (r) => r != null && r.trim().length > 0;
    ...
    try {
        const reissuable = (s) => s === "CANCELLED";
    ...
    try {
        ...
        const canQr = (s, remaining) => PAYABLE.includes(s) && remaining > 0;
    ...
    try {
        const validateBank = (bin, no, name) => {
            if (!bin || !no || !name)
                throw new Error("BANK_CONFIG_MISSING");
            return true;
        };
        try {
    ...
    try {
        const TENANT_VIS = ["ISSUED", "PARTIALLY_PAID", "PAID", "OVERDUE"];
        const tenantSee = (s) => TENANT_VIS.includes(s);
