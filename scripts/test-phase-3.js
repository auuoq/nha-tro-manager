...
const validate_charge_config_overlap_service_1 = require("../src/server/database/validate-charge-config-overlap.service");
const auth_service_1 = require("../src/server/auth/auth-service");
const building_charge_config_schema_1 = require("../src/features/buildings/schemas/building-charge-config.schema");
const building_schema_1 = require("../src/features/buildings/schemas/building.schema");
...
async function runPhase3Tests() {
    ...
    try {
        const tempPass = (0, auth_service_1.generateRandomTempPassword)(8);
        if (tempPass.length !== 8)
            throw new Error("Temp password length mismatch");
        const hashed = await (0, auth_service_1.hashPassword)(tempPass);
    ...
    try {
        const mockTx = {};
        await (0, validate_charge_config_overlap_service_1.validateChargeConfigOverlap)(mockTx, {
            buildingId: "b-1",
    ...
    try {
        const invalidRes = building_charge_config_schema_1.buildingChargeConfigSchema.safeParse({
            chargeType: client_1.ChargeType.WIFI,
            chargeMethod: client_1.ChargeMethod.METERED, // Invalid!
            unitPrice: 100000,
            effectiveFrom: new Date(),
        });
    ...
    try {
        const invalidFree = building_charge_config_schema_1.buildingChargeConfigSchema.safeParse({
            chargeType: client_1.ChargeType.PARKING,
            chargeMethod: client_1.ChargeMethod.FREE,
    ...
    try {
        const invalidDate = building_charge_config_schema_1.buildingChargeConfigSchema.safeParse({
    ...
    try {
        const invalidName = building_schema_1.createBuildingSchema.safeParse({
            name: "   ", // Only spaces
            address: "   ",
        });
